...
do
    ...
    # Remove old cache entry, if there is a chance the certificate files will no                                                                                                                                                                                                                                             t exist at some point the deletion can me marked as text
   python3  $SCRIPT_PATH -d $domain_base -r >> $LOG_PATH 2>&1

    # Add new certificate to cache
    python3 $SCRIPT_PATH -d $domain_base -k $KEY_PATH -c $CERT_PATH -C $CHAIN_PA                                                                                                                                                                                                                                             TH -a >> $LOG_PATH 2>&1
done
